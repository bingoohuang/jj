...
// Map returns a map of values. The result should be a JSON object.
...
func parseObject(c *parseContext, i int, path string, option *PathOption) (int, bool) {
	...
	for i < len(c.json) {
		...
		for ; i < len(c.json); i++ {
			...
			case '{':
				if pmatch && !hit {
					i, hit = parseObject(c, i+1, rp.path, option)
					if hit {
						return i, true
					}
				} else {
			...
			case '[':
				if pmatch && !hit {
					i, hit = parseArray(c, i+1, rp.path, option)
					if hit {
						return i, true
					}
				} else {
...
func Get(json, path string, optionsFns ...PathOptionFn) Result {
	...
	if !option.RawPath && len(path) >= 2 && path[0] == '.' && path[1] == '.' {
		...
		for ; i < len(c.json); i++ {
			if c.json[i] == '{' {
				i++
				parseObject(c, i, path, option)
				break
			}
			if c.json[i] == '[' {
				i++
				parseArray(c, i, path, option)
				break
			}
		}
	}
...
func modPretty(json, arg string) string {
	if len(arg) > 0 {
		...
		return bytesString(Pretty(stringBytes(json), opts))
	}
...
func getBytes(json []byte, path string, optionsFns ...PathOptionFn) Result {
	...
	if json != nil {
		...
		strh := sliceHeader{data: strhi.data, len: strhi.len, cap: strhi.len}
		if strh.data == nil {
			// str is nil
			if rawh.data == nil {
				// raw is nil
