...
const (
	...
	TokNumber  // the data is a JSON Number
	TokTrue    // the data is a JSON True
	TokFalse   // the data is a JSON False
	TokNull    // the data is a JSON NUll
	TokObject  // the data is a JSON Object (open or close character)
	TokArray   // the data is a JSON Array (open or close character)
	TokComma   // the data is a JSON comma character ','
	TokColon   // the data is a JSON colon character ':'
	TokStart   // the data is the start of the JSON document
	TokEnd     // the data is the end of the JSON document
	TokOpen    // the data is an open character (Object or Array, '{' or '[')
	TokClose   // the data is an close character (Object or Array, '}' or ']')
	TokKey     // the data is a JSON Object key
	TokValue   // the data is a JSON Object or Array value
	TokEscaped // the data is a String with at least one escape character ('\')
	TokSign    // the data is a signed Number (has a '-' prefix)
	TokDot     // the data is a Number has a dot (radix point)
	TokE       // the data is a Number in scientific notation (has 'E' or 'e')
)

// StreamParse Parse JSON.
...
// document.
func StreamParse(json []byte, iter func(start int, end int, info int) int) int {
...
func vdoc(json []byte, i int, f vfn) (oi int, ok, stop bool) {
	i, ok, stop = vany(json, i, TokStart, f)
	if stop {
		return i, ok, stop
	}
	for ; i < len(json); i++ {
...
func vany(json []byte, i int, dinfo int, f vfn) (oi int, ok, stop bool) {
	for ; i < len(json); i++ {
		...
		if json[i] == '"' {
			...
			f2 := f
			if f != nil {
				r := f(i, i+1, TokObject|TokOpen|dinfo)
				if r == 0 {
					return i, true, true
				}
				if r == -1 {
			...
			if f != nil {
				if dinfo&TokStart == TokStart {
					dinfo &= ^TokStart
					dinfo |= TokEnd
				}
				if f(i-1, i, TokObject|TokClose|dinfo) == 0 {
					return i, true, true
				}
			}
			return i, true, false
			...
			f2 := f
			if f != nil {
				r := f(i, i+1, TokArray|TokOpen|dinfo)
				if r == 0 {
					return i, true, true
				}
				if r == -1 {
			...
			if f != nil {
				...
				if f(i-1, i, TokArray|TokClose|dinfo) == 0 {
					return i, true, true
				}
			}
			return i, true, false
...
func vobject(json []byte, i int, f vfn) (oi int, ok, stop bool) {
	for ; i < len(json); i++ {
		...
		key:
			...
			if f != nil {
				if f(mark, i, info|TokKey|TokString) == 0 {
					return i, true, true
				}
			}
			...
			if f != nil {
				if f(i-1, i, TokColon) == 0 {
					return i, true, true
				}
			}
			if i, ok, stop = vany(json, i, TokValue, f); stop {
				return i, ok, stop
			}
