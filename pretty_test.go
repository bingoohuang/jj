...
func TestPretty(t *testing.T) {
	pretty := Pretty(Ugly(Pretty(example1)))
	...
	opts := DefaultOptions
	opts.SortKeys = true
	pretty = Pretty(Ugly(Pretty([]byte(example2))), opts)
	assertEqual(t, j(pretty), j(pretty))
	assertEqual(t, j(example2), j(pretty))
}
...
func TestBig(t *testing.T) {
  ...
  },
  {
	...
	opts := DefaultOptions
	opts.SortKeys = true
	jsonb := Pretty(Ugly([]byte(jso)), opts)
	assertEqual(t, j(jsonb), j(jso))
}
...
"arr":["1","2",1,2,true,false,null],
"obj":{"key1":null,"ar`+"\x1B[36m"+`Cyanr2":[1,2,3,"123","456"]}}
	`)), nil, &ColorOption{CountEntries: true})
	fmt.Printf("%s", res)
	if string(res) != `{[90m (4 items) [0m
  [94m"hello"[0m: [92m"world"[0m,
  [94m"what"[0m: [93m123[0m,
  [94m"arr"[0m: [[90m (7 items) [0m[92m"1"[0m, [92m"2"[0m, [93m1[0m, [93m2[0m, [96mtrue[0m, [96mfalse[0m, [91mnull[0m],
  [94m"obj"[0m: {[90m (2 items) [0m
    [94m"key1"[0m: [91mnull[0m,
    [94m"ar\u001b[36mCyanr2"[0m: [[90m (5 items) [0m[93m1[0m, [93m2[0m, [93m3[0m, [92m"123"[0m, [92m"456"[0m]
  }
}
` {
		t.Fatal("invalid output")
...
func TestPrettyColor(t *testing.T) {
	json := `"abc\u0020def\nghi"`
	ret := string(Color([]byte(json), nil, nil))
	exp := "" +
