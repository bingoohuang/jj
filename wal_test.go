...
func testLog(t *testing.T, opts *WalOptions, N int) {
	logPath := "testlog/" + strings.Join(strings.Split(t.Name(), "/")[1:], "/")
	l, err := WalOpen(logPath, opts)
	if err != nil {
		t.Fatal(err)
	}
	defer l.Close()
	...
	// WalOpen -- open log after truncating
	l, err = WalOpen(logPath, opts)
	if err != nil {
		t.Fatal(err)
	}
...
func testFirstLast(t *testing.T, l *WalLog, expectFirst, expectLast uint64, data func(index uint64) []byte) {
	t.Helper()
	fi, err := l.FirstIndex()
	if err != nil {
		t.Fatal(err)
...
func TestLog(t *testing.T) {
	...
	t.Run("no-sync", func(t *testing.T) {
		t.Run("json", func(t *testing.T) {
			testLog(t, makeOpts(512, true, JSONFormat), 100)
		})
		t.Run("binary", func(t *testing.T) {
			testLog(t, makeOpts(512, true, BinaryFormat), 100)
		})
	})
	t.Run("sync", func(t *testing.T) {
		t.Run("json", func(t *testing.T) {
			testLog(t, makeOpts(512, false, JSONFormat), 100)
		})
		t.Run("binary", func(t *testing.T) {
			testLog(t, makeOpts(512, false, BinaryFormat), 100)
		})
	})
}
...
func TestOutliers(t *testing.T) {
	...
	t.Run("fail-in-memory", func(t *testing.T) {
		if l, err := WalOpen(":memory:", nil); err == nil {
			l.Close()
	...
	t.Run("fail-not-a-directory", func(t *testing.T) {
		...
		if err := os.MkdirAll("testlog", 0o777); err != nil {
			...
			t.Fatal(err)
		} else if l, err := WalOpen("testlog/file", nil); err == nil {
			l.Close()
			t.Fatal("expected error")
		}
	})
	t.Run("load-with-junk-files", func(t *testing.T) {
		...
		f.Close()
		l, err := WalOpen("testlog/junk", nil)
		if err != nil {
			t.Fatal(err)
		}
		l.Close()
	...
	// 		n, _, err := readEntry(rd, JSONFormat, true)
	// 		if err == io.EOF {
	// 			break
	// 		}
	// 		if err != nil {
...
func TestConcurrency(t *testing.T) {
	...
	l, err := WalOpen("testlog", &WalOptions{
		NoSync: true,
		NoCopy: true,
	})
	if err != nil {
