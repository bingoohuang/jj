...
const (
	// BinaryFormat format writes entries in binary. This is the default and, unless
	// a good reason otherwise, should be used in production.
	BinaryFormat LogFormat = 0
	// JSONFormat format writes entries as JSON lines. This causes larger, human-readable files.
	JSONFormat LogFormat = 1
)

// WalOptions for WalLog
...
type WalLog struct {
	...
	scache     LRU        // segment entries cache
}
...
// WalOpen a new write ahead log
func WalOpen(path string, opts *WalOptions) (l *WalLog, err error) {
	if opts == nil {
		opts = DefaultWalOptions
	}
	if opts.SegmentCacheSize <= 0 {
		opts.SegmentCacheSize = DefaultWalOptions.SegmentCacheSize
	}
...
func (l *WalLog) pushCache(segIdx int) {
	_, _, _, v, evicted := l.scache.SetEvicted(segIdx, l.segments[segIdx])
	if evicted {
		s := v.(*segment)
		s.ebuf = nil
		s.epos = nil
...
func (l *WalLog) Write(index uint64, data []byte) error {
	l.mu.Lock()
	defer l.mu.Unlock()
	if l.corrupt {
		return ErrCorrupt
...
func (l *WalLog) appendEntry(dst []byte, index uint64, data []byte) (out []byte, epos bpos) {
	if l.opts.LogFormat == JSONFormat {
...
func (l *WalLog) WriteBatch(b *Batch) error {
	l.mu.Lock()
	defer l.mu.Unlock()
	if l.corrupt {
		return ErrCorrupt
...
// LastIndex returns the index of the last entry in the log. Returns 0 when no entries.
func (l *WalLog) LastIndex() (index uint64, err error) {
	l.mu.RLock()
	defer l.mu.RUnlock()
	if l.corrupt {
		return 0, ErrCorrupt
...
func (l *WalLog) findSegment(index uint64) int {
...
func loadNextJSONEntry(data []byte) (n int, err error) {
	...
	line := data[:idx]
	dres := Get(*(*string)(unsafe.Pointer(&line)), "data")
	if dres.Type != String {
		return 0, ErrCorrupt
	}
	return idx + 1, nil
...
func (l *WalLog) loadSegment(index uint64) (*segment, error) {
...
func (l *WalLog) Read(index uint64) (data []byte, err error) {
	l.mu.RLock()
	defer l.mu.RUnlock()
	if l.corrupt {
...
func readJSON(edata []byte) ([]byte, error) {
	var data []byte
	s := Get(*(*string)(unsafe.Pointer(&edata)), "data").String()
	if len(s) > 0 && s[0] == '$' {
		var err error
...
func (l *WalLog) ClearCache() error {
	l.mu.Lock()
	defer l.mu.Unlock()
	if l.corrupt {
		return ErrCorrupt
...
func (l *WalLog) TruncateFront(index uint64) error {
	l.mu.Lock()
	defer l.mu.Unlock()
	if l.corrupt {
		return ErrCorrupt
...
func (l *WalLog) truncateFront(index uint64) (err error) {
...
func (l *WalLog) TruncateBack(index uint64) error {
	l.mu.Lock()
	defer l.mu.Unlock()
	if l.corrupt {
		return ErrCorrupt
...
func (l *WalLog) truncateBack(index uint64) (err error) {
	if index == 0 || l.lastIndex == 0 ||
		index < l.firstIndex || index > l.lastIndex {
		return ErrOutOfRange
	}
